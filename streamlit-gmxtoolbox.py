...
class sasaplot(): 
    sasa_csv = "sasa-per-residue-matrix.csv"
    sasa_res = None


    def read_residue_labels_from_csv_header(self, df: pd.DataFrame) -> list[str]:
        cols = list(df.columns)
        if len(cols) < 2:
            raise ValueError("CSV must have at least 2 columns: time + at least one residue column.")
        return [str(c).strip() for c in cols[1:]]


    def load_residue_labels_from_map(path: str, n_res: int) -> list[str]:
        m = pd.read_csv(path, header=None)
        if m.shape[1] == 1:
            labels = m.iloc[:, 0].astype(str).tolist()
            if len(labels) != n_res:
                raise ValueError(f"Residue map has {len(labels)} labels but CSV has {n_res} residue columns.")
            return labels

        if m.shape[1] >= 2:
            labels = m.iloc[:, 1].astype(str).tolist()
            if len(labels) != n_res:
                raise ValueError(f"Residue map has {len(labels)} labels but CSV has {n_res} residue columns.")
            return labels

        raise ValueError("Residue map CSV must have 1 column (labels) or 2 columns (old,new).")
    ...
    def __init__(self, sasa_csv, sasa_csv_name, sasa_res, sasa_res_name, sasa_renumber_res_id, sasa_title):
        self.sasa_csv = sasa_csv
        self.sasa_res = sasa_res
        if sasa_renumber_res_id == 'None':
            self.renumber_res_id = sasa_renumber_res_id
        elif sasa_renumber_res_id is not None:
            self.renumber_res_id = int(sasa_renumber_res_id)

        df = pd.read_csv(sasa_csv)
        if df.shape[1] < 2:
            print("ERROR: CSV must contain time column + residue columns.", file=sys.stderr)
            sys.exit(1)

        time_col = df.columns[0]
        times = df[time_col].to_numpy(dtype=float)

        # SASA matrix: rows=time, cols=residues
        Z_abs = df.iloc[:, 1:].to_numpy(dtype=float)

        res_labels = self.read_residue_labels_from_csv_header(df)
        n_res = len(res_labels)

        if sasa_renumber_res_id != 'None':
            res_labels = [str(self.renumber_res_id + i) for i in range(n_res)]

        
        ### ✅ compute ΔSASA relative to first frame (row 0)
        baseline = Z_abs[0, :]                       # shape (n_res,)
        Z = Z_abs - baseline[None, :]                # broadcast to (n_time, n_res)

        # symmetric color range around 0 for fair red/blue comparison
        max_abs = float(np.nanmax(np.abs(Z)))
        zmin, zmax = -max_abs, max_abs

        # diverging colors: red -> white -> blue
        colorscale = [
            [0.0, "darkred"],
            [0.5, "white"],
            [1.0, "darkblue"],
        ...
        colorbar_title = "ΔSASA (nm²)"
        hover = "Time (ns): %{y}<br>Residue: %{x}<br>ΔSASA: %{z:.4f} nm²<extra></extra>"
        title = "Per-residue SASA heatmap (Δ from first frame)"
        fig = go.Figure(
            data=go.Heatmap(
                z=Z,
                x=res_labels,
        ...
        # 输出：你现在用的是 write_image（需要 kaleido）
        fig.write_image("/tmp/delta-sasa-matrix.png", width=1800, height=1000, scale=2)
        self.streamlit_download_file_plotly("delta-sasa-matrix.png", "/tmp/delta-sasa-matrix.png")
        ...
        colorbar_title = "SASA (nm²)"
        hover = "Time (ns): %{y}<br>Residue: %{x}<br>SASA: %{z:.4f} nm²<extra></extra>"
        title = "Per-residue SASA heatmap"
        fig = go.Figure(
            data=go.Heatmap(
                z=Z,
                x=res_labels,
        ...
        fig.update_yaxes(showgrid=False)
        # 输出：你现在用的是 write_image（需要 kaleido）
        fig.write_image("/tmp/sasa-matrix.png", width=1800, height=1000, scale=2)
        self.streamlit_download_file_plotly("sasa-matrix.png", "/tmp/sasa-matrix.png")
...
with mradder:
    ...
    ### 3rd subcolum in column 2, for plotting SASA/delta-SASA per-residue as the function of time ###
    # sasa-ploter.subheader("SASA per-Residue as the Function of Time (ns)\n " \
    mradder.subheader(
        """### SASA per-residue as a function of time (ns)

    mkdir -p sasa-per-residue

    # 1) Split trajectory: 1 frame per 1 ns (dt=1000 ps)
    gmx trjconv -f md_noPBC_dt1000_fit.xtc -s first.pdb -sep -dt 1000 -o sasa-per-residue/frame-.gro

    cd sasa-per-residue

    # 2) Compute per-residue SASA for each frame
    a=$(ls frame-*.gro 2>/dev/null | wc -l)
    for i in $(seq 0 $((a-1))); do
    echo 1 | gmx sasa -f "frame-${i}.gro" -s "frame-${i}.gro" -or "sasa-${i}.xvg" -o "area-${i}.xvg"
    done

    # Optional: remove frames to save space
    rm -f frame-*.gro

    # 3) Build CSV matrix: header + rows (Time(ns), SASA per residue)
    (
    # header: Time (ns),<residue indices...>
    printf "Time (ns)"
    awk '$1 ~ /^[0-9]+$/ {printf ",%s", $1} END{printf "\\n"}' sasa-0.xvg

    # data rows: time_index, sasa(res1), sasa(res2), ...
    i=0
    for f in $(ls -1v sasa-*.xvg); do
        printf "%d" "$i"
        awk '$1 ~ /^[0-9]+$/ {printf ",%s", $2} END{printf "\\n"}' "$f"
        i=$((i+1))
    done
    ) > ../sasa-per-residue-matrix.csv

    cd ..
    """)
    sasa_csv = st.file_uploader("Upload the sasa-per-residue-matrix.csv file", type=['csv'])
    sasa_res = st.file_uploader("Optional Choice, Upload the single frame pdb/gro file for residues info", type=['pdb', 'gro'])
    sasa_renumber_res_id =  st.text_input("Optional Choice, Renumber residue starting index from: ", 'None') 
    sasa_title =  st.text_input("Plot Title: ", 'Per-residue SASA heatmap') 

    # save the files name:
    if sasa_csv:
        sasa_csv_filename = sasa_csv.name
    if sasa_res:
        sasa_res_filename = sasa_res.name
    else:
        sasa_res_filename = None

    # 保存上传的文件到临时位置
    if st.button('SASA Plot') and sasa_csv:
        # 保存文件并获取临时文件路径
        sasa_csv_path = save_uploaded_file(sasa_csv)
        try:
            sasa_res_path = save_uploaded_file(sasa_res)
        except:
            sasa_res_path = None
            pass
        x = sasaplot(sasa_csv_path, sasa_csv_filename, sasa_res_path, sasa_res_filename, sasa_renumber_res_id, sasa_title)
...
#################################################################################################################################################
# Advanced structural analysis modules appended for Streamlit Cloud integration
#################################################################################################################################################

def adv_make_output_dir(prefix="gmx_toolbox"):
    import tempfile
    safe_prefix = re.sub(r"[^A-Za-z0-9_.-]+", "_", prefix).strip("_") or "gmx_toolbox"
    base_dir = "/tmp" if os.path.isdir("/tmp") else None
    return tempfile.mkdtemp(prefix=f"{safe_prefix}_", dir=base_dir)


def adv_read_binary_file(path):
    with open(path, "rb") as handle:
        return handle.read()


def adv_download_file(path, label, key):
    mime_type, _ = mimetypes.guess_type(path)
    st.download_button(
        label=label,
        data=adv_read_binary_file(path),
...
def adv_render_downloads(file_map, key_prefix):
    if not file_map:
        return
    st.markdown("**Download results**")
    for idx, (display_name, file_path) in enumerate(file_map.items()):
        adv_download_file(file_path, f"Download {display_name}", f"{key_prefix}_download_{idx}")


##########################################################################################
# Module 1: Batch PyMOL RMSD heatmap
##########################################################################################

def adv_check_pymol_available():
    try:
...
def adv_ensure_pymol_launched(pymol_module, cmd_module):
    try:
        cmd_module.get_version()
    except Exception:
        pymol_module.finish_launching(["pymol", "-qc"])


def adv_make_colormap():
    from matplotlib.colors import LinearSegmentedColormap
    return LinearSegmentedColormap.from_list(
        "deepblue_to_red",
        ["#0033CC", "#4F8CFF", "#F2F5FF", "#FFB3B3", "#FF0000"],
        N=256,
    )


def adv_build_discrete_colormap(base_cmap, vmin, vmax, interval):
    from matplotlib.colors import BoundaryNorm, ListedColormap
    if interval <= 0:
        raise ValueError("Color interval must be > 0 when discrete colorbar is enabled.")
    boundaries = np.arange(vmin, vmax + interval, interval, dtype=float)
    if len(boundaries) < 2:
        boundaries = np.array([vmin, vmax], dtype=float)
    if boundaries[-1] < vmax:
        boundaries = np.append(boundaries, vmax)
    n_bins = max(1, len(boundaries) - 1)
    colors = base_cmap(np.linspace(0, 1, n_bins))
    cmap = ListedColormap(colors, name=f"{base_cmap.name}_discrete_{n_bins}")
    norm = BoundaryNorm(boundaries, cmap.N, clip=True)
    return boundaries, cmap, norm


def adv_batch_pymol_pairwise_rmsd(cmd_module, obj_mobile, obj_target, selection, method, cycles, idx_i, idx_j):
    tmp_mobile = f"__tmp_mobile_{idx_i}_{idx_j}"
    tmp_target = f"__tmp_target_{idx_i}_{idx_j}"
    try:
        cmd_module.create(tmp_mobile, obj_mobile)
        cmd_module.create(tmp_target, obj_target)
        mob_sel = f"{tmp_mobile} and ({selection})"
        tar_sel = f"{tmp_target} and ({selection})"
        n_mob = cmd_module.count_atoms(mob_sel)
        n_tar = cmd_module.count_atoms(tar_sel)
        if n_mob == 0 or n_tar == 0:
            raise ValueError(
                f"Selection matched zero atoms (mobile={n_mob}, target={n_tar}). Selection used: {selection}"
            )
        if method == "align":
            rmsd = float(cmd_module.align(mob_sel, tar_sel, cycles=cycles, transform=1)[0])
        elif method == "super":
            rmsd = float(cmd_module.super(mob_sel, tar_sel, cycles=cycles, transform=1)[0])
        elif method == "cealign":
            rmsd = float(cmd_module.cealign(tar_sel, mob_sel, transform=1)["RMSD"])
        else:
            raise ValueError(f"Unsupported method: {method}")
    finally:
        for obj_name in (tmp_mobile, tmp_target):
            try:
                if obj_name in cmd_module.get_names("all"):
                    cmd_module.delete(obj_name)
            except Exception:
                pass
    return rmsd


def adv_batch_pymol_build_matrix(cmd_module, loaded, selection, method, cycles):
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            rmsd = adv_batch_pymol_pairwise_rmsd(
                cmd_module,
                loaded[i][1],
                loaded[j][1],
                selection,
...
def adv_batch_pymol_write_matrix_txt(labels, matrix, out_path):
    with open(out_path, "w", encoding="utf-8") as handle:
        handle.write("# Pairwise RMSD matrix (Angstrom)\n")
        handle.write("ID\t" + "\t".join(labels) + "\n")
        for label, row in zip(labels, matrix):
            values = "\t".join(f"{x:.4f}" for x in row)
            handle.write(f"{label}\t{values}\n")


def adv_batch_pymol_write_pairs_txt(labels, matrix, out_path):
    with open(out_path, "w", encoding="utf-8") as handle:
        handle.write("# Pairwise RMSD list (Angstrom)\n")
        handle.write("structure_1\tstructure_2\trmsd_angstrom\n")
        n = len(labels)
        for i in range(n):
            for j in range(i + 1, n):
                handle.write(f"{labels[i]}\t{labels[j]}\t{matrix[i, j]:.4f}\n")


def adv_batch_pymol_write_matrix_csv(labels, matrix, out_path):
...
def adv_batch_pymol_plot_heatmap(labels, matrix, out_png, vmin, vmax, annot, dpi, discrete_colorbar=False, color_interval=0.5):
    base_cmap = adv_make_colormap()
    n = len(labels)
    vmax = float(np.max(matrix)) if vmax is None else float(vmax)
    if vmax <= vmin:
        vmax = vmin + 1e-6
    fig_size = max(7.0, 0.55 * n + 2.5)
    fig, ax = plt.subplots(figsize=(fig_size, fig_size))
    if discrete_colorbar:
        boundaries, cmap, norm = adv_build_discrete_colormap(base_cmap, vmin, vmax, color_interval)
        im = ax.imshow(matrix, cmap=cmap, norm=norm, interpolation="nearest")
    else:
        boundaries = None
        im = ax.imshow(matrix, cmap=base_cmap, vmin=vmin, vmax=vmax, interpolation="nearest")
    ...
    if annot:
        ...
        for i in range(n):
            for j in range(n):
                val = matrix[i, j]
                text_color = "black" if mid > val else "white"
    ...
    if discrete_colorbar:
        ...
        tick_labels = [f"{x:.2f}".rstrip("0").rstrip(".") for x in boundaries]
        cbar.ax.set_yticklabels(tick_labels)
    else:
        cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_label("RMSD (Å)", rotation=270, labelpad=15)
    ax.set_title("Pairwise RMSD heatmap", pad=28)
    plt.tight_layout()
...
def adv_run_batch_pymol_rmsd_heatmap(pdb_paths, pdb_names, output_prefix, selection, method, cycles, vmin, vmax, annot, dpi, discrete_colorbar, color_interval):
    try:
        ...
        raise RuntimeError(
            "PyMOL Python module is not available in the current deployment environment. "
            f"Original import error: {exc}"
        ) from exc

    adv_ensure_pymol_launched(pymol, cmd)
    cmd.reinitialize()
    if len(pdb_paths) < 2:
        raise ValueError("At least 2 PDB files are required for pairwise RMSD calculation.")
    ...
    matrix_txt = f"{output_prefix}_matrix.txt"
    pairs_txt = f"{output_prefix}_pairs.txt"
    matrix_csv = f"{output_prefix}_matrix.csv"
    heatmap_png = f"{output_prefix}_heatmap.png"

    adv_batch_pymol_write_matrix_txt(labels, matrix, matrix_txt)
    adv_batch_pymol_write_pairs_txt(labels, matrix, pairs_txt)
    adv_batch_pymol_write_matrix_csv(labels, matrix, matrix_csv)
    adv_batch_pymol_plot_heatmap(
        labels,
        matrix,
        heatmap_png,
        float(vmin),
        ...
        bool(annot),
        int(dpi),
        discrete_colorbar=bool(discrete_colorbar),
        color_interval=float(color_interval),
    )
...
def adv_render_batch_pymol_module():
    ...
    if not pymol_ok:
        st.warning(
            "PyMOL is not available in the current runtime, so this module may not run on Streamlit Cloud unless PyMOL is installed there. "
            f"Import error: {pymol_error}"
        )
    ...
    selection = st.text_input(
        "Atom selection",
        value="polymer.protein and name N+CA+C+O",
        key="adv_batch_pymol_selection",
    )
    method = st.selectbox("Method", ["align", "super", "cealign"], key="adv_batch_pymol_method")
    cycles = st.number_input("Cycles", min_value=0, step=1, value=0, key="adv_batch_pymol_cycles")
    vmin = st.number_input("vmin", value=0.0, step=0.1, key="adv_batch_pymol_vmin")
    use_custom_vmax = st.checkbox("Set custom vmax", value=False, key="adv_batch_pymol_use_vmax")
    vmax = st.number_input("vmax", value=5.0, step=0.1, key="adv_batch_pymol_vmax") if use_custom_vmax else None
    annot = st.checkbox("Annotate heatmap cells", value=False, key="adv_batch_pymol_annot")
    dpi = st.number_input("DPI", min_value=72, step=50, value=300, key="adv_batch_pymol_dpi")
    discrete_colorbar = st.checkbox("Use discrete colorbar", value=False, key="adv_batch_pymol_discrete")
    color_interval = st.number_input(
        "Color interval",
        min_value=0.01,
        step=0.1,
        value=0.5,
        key="adv_batch_pymol_color_interval",
    )

    if st.button("Run batch PyMOL RMSD", key="adv_batch_pymol_run"):
        if not pdb_files or len(pdb_files) < 2:
            st.error("Please upload at least 2 PDB files.")
            return
        run_dir = adv_make_output_dir("batch_pymol_rmsd")
        pdb_paths = []
        for uploaded in pdb_files:
        ...
        try:
            with st.spinner("Running PyMOL pairwise RMSD calculation..."):
                outputs, labels, matrix = adv_run_batch_pymol_rmsd_heatmap(
                    ...
                    bool(annot),
                    int(dpi),
                    bool(discrete_colorbar),
                    float(color_interval),
                )
            st.success("Batch PyMOL RMSD calculation completed.")
            st.image(outputs["heatmap_png"], caption="Pairwise RMSD heatmap")
            st.dataframe(pd.DataFrame(matrix, index=labels, columns=labels))
            adv_render_downloads(outputs, "adv_batch_pymol")
        except Exception as exc:
            st.error(f"Batch PyMOL RMSD failed: {exc}")


##########################################################################################
# Module 2: Density-colored scatter from COLVAR
##########################################################################################

def adv_load_colvar(path):
    data = np.loadtxt(path, comments="#")
    if data.ndim == 1:
        data = np.atleast_2d(data)
    if data.shape[1] < 3:
        raise ValueError("COLVAR file must contain at least 3 numeric columns: time, RMSD, contact points.")
    rmsd = data[:, 1]
    contacts_values = data[:, 2]
    return rmsd, contacts_values


def adv_density_scatter(rmsd, contacts_values, bins=100, out_png="contacts_vs_rmsd_density_scatter.png", x_range=None, y_range=None):
    mask = np.isfinite(rmsd) & np.isfinite(contacts_values)
    if x_range is not None:
        mask &= (contacts_values >= x_range[0]) & (contacts_values <= x_range[1])
    if y_range is not None:
        mask &= (rmsd >= y_range[0]) & (rmsd <= y_range[1])
    rmsd = rmsd[mask]
    contacts_values = contacts_values[mask]
    if rmsd.size == 0 or contacts_values.size == 0:
        raise ValueError("No valid points remain after filtering. Please loosen x/y ranges or check the file content.")

    H, xedges, yedges = np.histogram2d(contacts_values, rmsd, bins=bins)
    x_idx = np.searchsorted(xedges, contacts_values) - 1
    y_idx = np.searchsorted(yedges, rmsd) - 1
    x_idx = np.clip(x_idx, 0, H.shape[0] - 1)
    y_idx = np.clip(y_idx, 0, H.shape[1] - 1)
    density = np.log10(H[x_idx, y_idx] + 1)
    idx_sort = density.argsort()
    contacts_values = contacts_values[idx_sort]
    rmsd = rmsd[idx_sort]
    density = density[idx_sort]
...
def adv_run_density_scatter(colvar_path, bins, output_png, x_range=None, y_range=None):
    rmsd, contacts_values = adv_load_colvar(colvar_path)
    adv_density_scatter(
        rmsd,
        contacts_values,
        bins=int(bins),
    ...
    return output_png, len(rmsd)


def adv_render_density_scatter_module():
    st.subheader("Density-colored scatter from COLVAR")
    st.caption("Upload a COLVAR-like text file, read RMSD and contact points, and generate a density-colored scatter PNG.")
    colvar_file = st.file_uploader("Upload COLVAR file", type=None, key="adv_density_colvar")
    bins = st.number_input("Bins", min_value=10, step=10, value=100, key="adv_density_bins")
    output_png_name = st.text_input(
        "Output PNG name",
        value="contacts_vs_rmsd_density_scatter.png",
        key="adv_density_output_name",
    )
    use_x_range = st.checkbox("Enable x_range", value=False, key="adv_density_use_x_range")
    if use_x_range:
        x_col1, x_col2 = st.columns(2)
        with x_col1:
            x_min = st.number_input("x_range min", value=0.0, step=0.1, key="adv_density_x_min")
        with x_col2:
            x_max = st.number_input("x_range max", value=100.0, step=0.1, key="adv_density_x_max")
        x_range = [x_min, x_max]
    else:
        x_range = None
    use_y_range = st.checkbox("Enable y_range", value=False, key="adv_density_use_y_range")
    if use_y_range:
        y_col1, y_col2 = st.columns(2)
        with y_col1:
            y_min = st.number_input("y_range min", value=0.0, step=0.1, key="adv_density_y_min")
        with y_col2:
            y_max = st.number_input("y_range max", value=2.5, step=0.1, key="adv_density_y_max")
        y_range = [y_min, y_max]
    else:
        y_range = None

    if st.button("Run density scatter", key="adv_density_run"):
        if not colvar_file:
            st.error("Please upload a COLVAR file.")
            return
        if x_range is not None and x_range[0] > x_range[1]:
            st.error("x_range min must be <= x_range max.")
            return
        if y_range is not None and y_range[0] > y_range[1]:
            st.error("y_range min must be <= y_range max.")
            return
        colvar_path = save_uploaded_file(colvar_file)
        if not colvar_path:
            st.error("Failed to save the uploaded COLVAR file.")
            return
        run_dir = adv_make_output_dir("density_scatter")
        output_png = os.path.join(run_dir, os.path.basename(output_png_name) or "contacts_vs_rmsd_density_scatter.png")
        try:
            with st.spinner("Generating density-colored scatter plot..."):
                out_png, point_count = adv_run_density_scatter(
                    colvar_path,
                    int(bins),
                    output_png,
                    x_range=x_range,
            ...
            st.success(f"Density scatter completed. Parsed {point_count} rows.")
            st.image(out_png, caption="Density-colored scatter plot")
            adv_render_downloads({"density_scatter_png": out_png}, "adv_density")
        except Exception as exc:
            st.error(f"Density scatter failed: {exc}")


##########################################################################################
# Module 3: Cross-RMSD heatmap between two trajectories
##########################################################################################

def adv_cross_require_topology(traj_path, top_path, argname):
    ext = os.path.splitext(traj_path)[1].lower()
    coord_only = {".xtc", ".trr", ".dcd", ".nc"}
    if ext in coord_only and top_path is None:
        raise ValueError(f"{argname} is required for trajectory '{os.path.basename(traj_path)}' because {ext} does not contain topology.")


def adv_cross_load_universe(top_path, traj_path):
    if top_path is None:
        return mda.Universe(traj_path, traj_path)
    return mda.Universe(top_path, traj_path)


def adv_cross_frame_indices(n_total, stride, max_frames):
    ids = list(range(0, n_total, stride))
    if max_frames is not None:
        ids = ids[:max_frames]
    return ids


def adv_cross_load_coords(top_path, traj_path, atom_selection, stride=1, max_frames=None, dtype=np.float64):
    universe = adv_cross_load_universe(top_path, traj_path)
    atom_group = universe.select_atoms(atom_selection)
    if atom_group.n_atoms == 0:
        raise RuntimeError(f"Selection returned 0 atoms for {os.path.basename(traj_path)}: {atom_selection}")
    ids = adv_cross_frame_indices(len(universe.trajectory), stride, max_frames)
    coords = np.empty((len(ids), atom_group.n_atoms, 3), dtype=dtype)
    for i, frame_index in enumerate(ids):
        universe.trajectory[frame_index]
        coords[i] = atom_group.positions.astype(dtype, copy=False)
    coords -= coords.mean(axis=1, keepdims=True)
    return coords


def adv_cross_load_reference(ref_file, atom_selection, expected_atoms, dtype=np.float64):
    universe = mda.Universe(ref_file)
    atom_group = universe.select_atoms(atom_selection)
    if atom_group.n_atoms == 0:
        raise RuntimeError(f"Reference selection returned 0 atoms: {atom_selection}")
    if atom_group.n_atoms != expected_atoms:
        raise RuntimeError(
            f"Reference atom count ({atom_group.n_atoms}) does not match trajectory selection ({expected_atoms}). "
            f"Make sure reference/topologies share the same atom order for selection '{atom_selection}'."
        )
    ref = atom_group.positions.astype(dtype, copy=False)
    ref -= ref.mean(axis=0, keepdims=True)
    return ref


def adv_cross_kabsch_fit(P, Q):
    covariance = P.T @ Q
...
def adv_cross_kabsch_rotation(P, Q):
    covariance = P.T @ Q
    V, _, Wt = np.linalg.svd(covariance)
    d = np.sign(np.linalg.det(V @ Wt))
    D = np.diag([1.0, 1.0, d])
...
def adv_cross_align_to_reference(coords, ref):
    out = np.empty_like(coords)
    for i in range(coords.shape[0]):
        out[i] = adv_cross_kabsch_fit(coords[i], ref)
    return out


def adv_cross_rmsd_matrix_common_frame(A, B):
    nA, n_atoms_A, _ = A.shape
    nB, n_atoms_B, _ = B.shape
    if n_atoms_A != n_atoms_B:
        raise RuntimeError(f"Atom count mismatch: traj1 has {n_atoms_A}, traj2 has {n_atoms_B}.")
    matrix = np.empty((nB, nA), dtype=np.float32)
    for j in range(nB):
        diff = A - B[j]
        matrix[j] = np.sqrt((diff * diff).sum(axis=(1, 2)) / n_atoms_A, dtype=np.float64)
    return matrix


def adv_cross_rmsd_matrix_pairwise_fit(A, B, block_size=64, progress_every=25, same_traj=False):
    nA, n_atoms_A, _ = A.shape
    nB, n_atoms_B, _ = B.shape
    if n_atoms_A != n_atoms_B:
        raise RuntimeError(f"Atom count mismatch: traj1 has {n_atoms_A}, traj2 has {n_atoms_B}.")
    ...
    matrix = np.empty((nB, nA), dtype=np.float32)
    if same_traj and nA == nB:
        for j in range(nB):
            matrix[j, j] = 0.0
            for i0 in range(j + 1, nA, block_size):
                i1 = min(i0 + block_size, nA)
                for i in range(i0, i1):
                    rotation = adv_cross_kabsch_rotation(A[i], B[j])
                    diff = A[i] @ rotation - B[j]
                    value = np.sqrt((diff * diff).sum() / n_atoms_A)
                    matrix[j, i] = value
                    matrix[i, j] = value
        return matrix
    for j in range(nB):
        Q = B[j]
        for i0 in range(0, nA, block_size):
            i1 = min(i0 + block_size, nA)
            for i in range(i0, i1):
                rotation = adv_cross_kabsch_rotation(A[i], Q)
                diff = A[i] @ rotation - Q
                matrix[j, i] = np.sqrt((diff * diff).sum() / n_atoms_A)
    return matrix


def adv_cross_build_extent(n_frames, stride, dt_ps, time_unit):
    if time_unit == "frame" or dt_ps is None:
        return (0, n_frames - 1), "Frame index"
    total_ps = (n_frames - 1) * stride * dt_ps
    if time_unit == "ps":
        return (0.0, total_ps), "Time (ps)"
    return (0.0, total_ps / 1000.0), "Time (ns)"


def adv_cross_plot_heatmap(matrix, out_png, title, xlabel, ylabel, extent_x, extent_y, cmap="viridis", vmax_percentile=99.0, dpi=600):
    ...
    axis.tick_params(which="major", length=7, width=1.5)
    axis.tick_params(which="minor", length=4, width=1.0)
    plt.colorbar(im, label=f"RMSD (Å)  (vmax=p{vmax_percentile:.0f})")
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
    plt.tight_layout()
...
def adv_run_cross_rmsd(traj1_path, traj2_path, top1_path=None, top2_path=None, shared_top_path=None, ref_path=None,
                       selection="protein and name CA", stride1=1, stride2=1, max_frames1=None, max_frames2=None,
                       dt1_ps=None, dt2_ps=None, time_unit="ns", use_pairwise=False, pairwise_block_size=64,
                       progress_every=25, use_float32=False, vmax_percentile=99.0, dpi=600, cmap="viridis",
                       title=None, output_prefix="cross_rmsd"):
    top1 = top1_path or shared_top_path
    top2 = top2_path or shared_top_path
    adv_cross_require_topology(traj1_path, top1, "top1/shared top")
    adv_cross_require_topology(traj2_path, top2, "top2/shared top")
    dtype = np.float32 if use_float32 else np.float64

    A = adv_cross_load_coords(top1, traj1_path, selection, stride1, max_frames1, dtype=dtype)
    B = adv_cross_load_coords(top2, traj2_path, selection, stride2, max_frames2, dtype=dtype)
    if B.shape[1] != A.shape[1]:
        raise RuntimeError(
            f"Selection atom count mismatch between traj1 ({A.shape[1]}) and traj2 ({B.shape[1]})."
        )

    if use_pairwise:
        if ref_path:
            ref = adv_cross_load_reference(ref_path, selection, A.shape[1], dtype=dtype)
            A = adv_cross_align_to_reference(A, ref)
            B = adv_cross_align_to_reference(B, ref)
        same_traj = (
            os.path.abspath(traj1_path) == os.path.abspath(traj2_path)
            and os.path.abspath(top1 or "") == os.path.abspath(top2 or "")
            and stride1 == stride2
            and max_frames1 == max_frames2
        )
        matrix = adv_cross_rmsd_matrix_pairwise_fit(
            A,
            B,
        ...
        if not ref_path:
            raise ValueError("Reference structure is required in common-reference mode.")
        ref = adv_cross_load_reference(ref_path, selection, A.shape[1], dtype=dtype)
        A = adv_cross_align_to_reference(A, ref)
        B = adv_cross_align_to_reference(B, ref)
        matrix = adv_cross_rmsd_matrix_common_frame(A, B)

    extent_x, xlabel_base = adv_cross_build_extent(A.shape[0], stride1, dt1_ps, time_unit)
    extent_y, ylabel_base = adv_cross_build_extent(B.shape[0], stride2, dt2_ps, time_unit)
    xlabel = f"{os.path.basename(traj1_path)} ({xlabel_base})"
    ylabel = f"{os.path.basename(traj2_path)} ({ylabel_base})"
    plot_title = title or "Cross-RMSD heatmap"
    out_png = f"{output_prefix}.png"
    out_npy = f"{output_prefix}.npy"
    adv_cross_plot_heatmap(
        matrix,
        out_png=out_png,
        title=plot_title,
        xlabel=xlabel,
    ...
    return {"heatmap_png": out_png, "matrix_npy": out_npy}, matrix


def adv_render_cross_rmsd_module():
    st.subheader("Cross-RMSD heatmap between two trajectories")
    st.caption("Upload two trajectories plus topology/reference inputs, then compute a cross-RMSD matrix in common-reference or pairwise-fit mode.")
    traj1 = st.file_uploader("Upload traj1", type=["xtc", "trr", "dcd", "nc", "pdb", "gro"], key="adv_cross_traj1")
    traj2 = st.file_uploader("Upload traj2", type=["xtc", "trr", "dcd", "nc", "pdb", "gro"], key="adv_cross_traj2")
    shared_top = st.file_uploader("Upload shared topology (optional)", type=["pdb", "gro", "tpr", "psf"], key="adv_cross_shared_top")
    top1 = st.file_uploader("Upload top1 (optional)", type=["pdb", "gro", "tpr", "psf"], key="adv_cross_top1")
    top2 = st.file_uploader("Upload top2 (optional)", type=["pdb", "gro", "tpr", "psf"], key="adv_cross_top2")
    ref = st.file_uploader("Upload reference structure (optional in pairwise-fit, required in common-reference mode)", type=["pdb", "gro"], key="adv_cross_ref")

    selection = st.text_input("Selection", value="protein and name CA", key="adv_cross_selection")
    stride_col1, stride_col2 = st.columns(2)
    with stride_col1:
        stride1 = st.number_input("stride1", min_value=1, step=1, value=1, key="adv_cross_stride1")
    with stride_col2:
        stride2 = st.number_input("stride2", min_value=1, step=1, value=1, key="adv_cross_stride2")

    max_col1, max_col2 = st.columns(2)
    with max_col1:
        use_max_frames1 = st.checkbox("Set max_frames1", value=False, key="adv_cross_use_max_frames1")
        max_frames1 = st.number_input("max_frames1", min_value=1, step=1, value=100, key="adv_cross_max_frames1") if use_max_frames1 else None
    with max_col2:
        use_max_frames2 = st.checkbox("Set max_frames2", value=False, key="adv_cross_use_max_frames2")
        max_frames2 = st.number_input("max_frames2", min_value=1, step=1, value=100, key="adv_cross_max_frames2") if use_max_frames2 else None

    dt_col1, dt_col2 = st.columns(2)
    with dt_col1:
        use_dt1 = st.checkbox("Set dt1_ps", value=False, key="adv_cross_use_dt1")
        dt1_ps = st.number_input("dt1_ps", min_value=0.0, step=0.1, value=1.0, key="adv_cross_dt1") if use_dt1 else None
    with dt_col2:
        use_dt2 = st.checkbox("Set dt2_ps", value=False, key="adv_cross_use_dt2")
        dt2_ps = st.number_input("dt2_ps", min_value=0.0, step=0.1, value=1.0, key="adv_cross_dt2") if use_dt2 else None

    time_unit = st.selectbox("time_unit", ["frame", "ps", "ns"], index=2, key="adv_cross_time_unit")
    mode = st.radio(
        "Mode",
        ["common-reference mode", "pairwise-fit"],
        index=0,
        key="adv_cross_mode",
    )
    use_pairwise = mode == "pairwise-fit"
    if use_pairwise:
        st.info("Pairwise-fit mode is mathematically stricter but can be much slower for large trajectories.")
    else:
        st.info("Common-reference mode is faster, but requires a reference structure.")

    pairwise_block_size = st.number_input("pairwise_block_size", min_value=1, step=1, value=64, key="adv_cross_pairwise_block_size")
    progress_every = st.number_input("progress_every", min_value=0, step=1, value=25, key="adv_cross_progress_every")
    use_float32 = st.checkbox("Use float32 to reduce memory", value=False, key="adv_cross_float32")
    vmax_percentile = st.number_input("vmax_percentile", min_value=1.0, max_value=100.0, step=1.0, value=99.0, key="adv_cross_vmax_percentile")
    dpi = st.number_input("DPI", min_value=72, step=50, value=600, key="adv_cross_dpi")
    cmap = st.text_input("Colormap", value="viridis", key="adv_cross_cmap")
    title = st.text_input("Custom title", value="Cross-RMSD heatmap", key="adv_cross_title")
    output_prefix = st.text_input("Output prefix", value="cross_rmsd", key="adv_cross_output_prefix")

    if st.button("Run cross-RMSD", key="adv_cross_run"):
        if not traj1 or not traj2:
            st.error("Please upload both traj1 and traj2.")
            return
        traj1_ext = os.path.splitext(traj1.name)[1].lower()
        traj2_ext = os.path.splitext(traj2.name)[1].lower()
        coord_only = {".xtc", ".trr", ".dcd", ".nc"}
        if traj1_ext in coord_only and not (top1 or shared_top):
            st.error("traj1 is a coordinate-only trajectory, so you must upload top1 or a shared topology.")
            return
        if traj2_ext in coord_only and not (top2 or shared_top):
            st.error("traj2 is a coordinate-only trajectory, so you must upload top2 or a shared topology.")
            return
        if not use_pairwise and not ref:
            st.error("Reference structure is required in common-reference mode.")
            return
        traj1_path = save_uploaded_file(traj1)
        traj2_path = save_uploaded_file(traj2)
        shared_top_path = save_uploaded_file(shared_top) if shared_top else None
        top1_path = save_uploaded_file(top1) if top1 else None
        top2_path = save_uploaded_file(top2) if top2 else None
        ref_path = save_uploaded_file(ref) if ref else None
        if not traj1_path or not traj2_path:
            st.error("Failed to save the uploaded trajectory files.")
            return
        run_dir = adv_make_output_dir("cross_rmsd")
        out_prefix = os.path.join(run_dir, re.sub(r"[^A-Za-z0-9_.-]+", "_", output_prefix).strip("_") or "cross_rmsd")
        try:
            with st.spinner("Computing cross-RMSD matrix..."):
                outputs, matrix = adv_run_cross_rmsd(
                    traj1_path=traj1_path,
                    traj2_path=traj2_path,
                    top1_path=top1_path,
                    top2_path=top2_path,
                    shared_top_path=shared_top_path,
                    ref_path=ref_path,
                    selection=selection,
                    stride1=int(stride1),
                    stride2=int(stride2),
                    max_frames1=max_frames1,
                    max_frames2=max_frames2,
                    dt1_ps=dt1_ps,
                    dt2_ps=dt2_ps,
                    time_unit=time_unit,
            ...
            st.success(f"Cross-RMSD completed. Matrix shape: {matrix.shape[0]} x {matrix.shape[1]}")
            st.image(outputs["heatmap_png"], caption="Cross-RMSD heatmap")
            st.caption("For large matrices, only the saved .npy file is offered for download rather than rendering the full array in the app.")
            adv_render_downloads(outputs, "adv_cross")
        except Exception as exc:
            st.error(f"Cross-RMSD failed: {exc}")


#################################################################################################################################################
# Full-width section for the three newly integrated modules
#################################################################################################################################################

st.markdown("---")
st.header("Advanced structural analysis")
st.caption("Three newly integrated standalone modules are provided below. Each module has independent inputs, execution logic, and downloadable outputs.")

with st.expander("1) Batch all-vs-all RMSD heatmap (PyMOL)", expanded=False):
    adv_render_batch_pymol_module()

with st.expander("2) Density-colored scatter from COLVAR", expanded=False):
    adv_render_density_scatter_module()

with st.expander("3) Cross-RMSD heatmap between two trajectories", expanded=False):
    adv_render_cross_rmsd_module()
